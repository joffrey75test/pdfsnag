...
projectsRouter.get("/", async (c) => {
  ...
  const rows = await c.env.DB.prepare(
    `SELECT pm.project_id, p.name, p.company_id, pm.role, pm.status
     FROM project_memberships pm
     JOIN projects p ON p.project_id = pm.project_id
     WHERE pm.user_id = ? AND pm.status = 'active' AND p.company_id = ?
     ORDER BY p.name ASC, pm.project_id ASC`
  )
...
projectsRouter.post("/:projectId/doc-folders/:folderId/permissions", requireProjectRole("manager"), async (c) => {
  ...
  if (!folder?.folder_id) return c.json({ ok: false, error: "Folder not found in folders." }, 404);
...
projectsRouter.get("/:projectId/doc-folders/permissions", requireProjectRole("manager"), async (c) => {
  ...
  const rows = await c.env.DB.prepare(
    `SELECT p.project_id, p.folder_id, f.path, p.user_id, p.can_read, p.can_write, p.created_at, p.updated_at
     ...
     JOIN folders f ON f.id = p.folder_id AND f.project_id = p.project_id
